...
class FileCrypto2(private val context: Context) {
     ...
     */
    fun encryptFileDataPasswordBasedAdvanced(fileData: ByteArray, metadata: String, password: String): ByteArray {
        try {
            ...
            val encryptedContent = cipher.doFinal(combinedData.toString().toByteArray(Charsets.UTF_8))
     ...
     */
    fun decryptFileDataPasswordBasedAdvanced(encryptedData: ByteArray, password: String): Pair<ByteArray, String> {
        try {
            // Minimum size check
    ...
    fun encryptFileDataRSAAdvanced(
        ...
        try {
            ...
            val encryptedString = if (enablePFS && enableSignatures) {
                val keyPair = userKeyPair?.let { 
                    java.security.KeyPair(it.first, it.second)
                }
                rsaCrypto.encryptRSA4096WithAESFull(
                    combinedString, 
                    recipientPublicKey, 
                    if (expirationTime > 0L) expirationTime else 0L, // Use provided expiration time
                    keyPair
                )
            } else {
                val keyPair = userKeyPair?.let { 
                ...
                rsaCrypto.encryptRSAWithAllFeatures(
                    combinedString,
                    recipientPublicKey,
                    enablePFS,
                    enableSignatures,
    ...
    fun decryptFileDataRSAAdvanced(
        ...
        try {
            ...
            val decryptedString = rsaCrypto.decryptRSAMessage(encryptedString, privateKey, senderPublicKey)
     ...
     */
    fun encryptFileDataBurnAfterReading(fileData: ByteArray, metadata: String, password: String): ByteArray {
        try {
