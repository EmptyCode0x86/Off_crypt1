...
class PasswordCrypto {
    ...
    /**
     * Encrypts a message using password-based encryption with multiple salts
     * Uses separate salts for master, encryption, and MAC keys
     */
     ...
     */
    private fun encryptPasswordBasedMultiSalt(message: String, password: String, expirationTime: Long): String {
        try {
            ...
            val encryptionKey = password2.generateSecureKey(password, encryptionSalt, ITERATION_COUNT)
    ...
    private fun decryptPasswordBasedLegacy(encryptedData: ByteArray, password: String): String {
        try {
            ...
            // Extract timestamps
            val creationTime = mainBuffer.getLong()
    ...
    private fun verifyHMAC(stored: ByteArray, calculated: ByteArray): Boolean {
        if (stored.size != calculated.size) return false
        var result = 0
        for (i in stored.indices) {
            result = result or (stored[i].toInt() xor calculated[i].toInt())
        }
        return result == 0
    }
    ...
    // Delegate missing functions to Password2
    private fun generateSecureKey(password: String, salt: ByteArray, iterations: Int): ByteArray {
        return password2.generateSecureKey(password, salt, iterations)
    }
    ...
    private fun verifyAdvancedHMAC(data: ByteArray, storedMac: ByteArray, macKey: ByteArray): Boolean {
        return password2.verifyAdvancedHMAC(data, storedMac, macKey)
    }
    
    private fun createAdvancedMetadata(message: String, timestamp: Long, expirationTime: Long): String {
